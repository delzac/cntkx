...
def test_scaled_dot_product_attention1():
    ...
    n1 = np.random.random((4, 5)).astype(np.float32)
    n2 = np.random.random((2, 5)).astype(np.float32)

    results = b.eval({a: [n1, n2]})
    assert_equal(np.sum(results[1][2:]), 0)  # check that n2 after seq_len 2 is empty
